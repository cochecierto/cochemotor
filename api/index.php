...
try {
    ...
    if ($route==='/api/vehicles' && $method==='GET') {
        ...
        foreach($q->fetchAll() as $row){
            ...
            $items[]=['id'=>$row['vehicle_id'],'userId'=>$u['user_id'],'brand'=>$row['brand'],'model'=>$row['model'],'version'=>$row['version'],'year'=>(int)$row['year'],'km'=>number_format((int)$row['mileage_km'],0,',','.').' km','price'=>(float)$row['cash_price'],'badge'=>$row['dgt_badge'],'stage'=>$row['stage'],'status'=>$row['status'],'evidenceLevel'=>$row['evidence_level'],'public_slug'=>$row['public_slug'],'fuel'=>(string)($metadata['fuel']??''),'gearbox'=>(string)($metadata['gearbox']??''),'location'=>(string)($metadata['location']??''),'province'=>(string)($metadata['province']??''),'pricingAnalysis'=>(is_array($metadata['pricingAnalysis']??null)?$metadata['pricingAnalysis']:[]),'images'=>$images,'image'=>$images[0]??'assets/brand/icons/vehicle-placeholder.svg','isDemo'=>false];
        }
        jsonResponse(['vehicles'=>$items]);
    }
...
function ensureOAuthSchema(PDO $pdo): void {
    ...
    if ($route==='/api/vehicles' && $method==='POST' && str_starts_with(strtolower($_SERVER['CONTENT_TYPE']??''),'multipart/form-data')) {
        ...
        $brand=trim((string)$v['brand']); $model=trim((string)$v['model']); $version=trim((string)($v['version']??'')); $yearValue=$v['year']; $kmValue=$v['km']; $priceValue=$v['price']; $year=(int)$yearValue; $km=(int)$kmValue; $price=(float)$priceValue; $publicationStatus=(string)($v['publicationStatus']??'disponible'); $pricingInput=is_array($v['pricingAnalysis']??null)?$v['pricingAnalysis']:[]; $pricingAnalysis=[]; foreach(['purchasePrice','auctionFee','acquisitionCosts','preparationCosts','riskCosts','minimumProfit','totalAcquisitionCost','quickSalePrice','optimalSalePrice','premiumSalePrice','maximumPurchasePrice','maximumAuctionBid'] as $pricingField){if(isset($pricingInput[$pricingField])&&is_numeric($pricingInput[$pricingField]))$pricingAnalysis[$pricingField]=max(0,(float)$pricingInput[$pricingField]);} foreach(['purchaseSource','taxMode','recommendedStrategy','riskLevel'] as $pricingField){if(isset($pricingInput[$pricingField])&&is_scalar($pricingInput[$pricingField]))$pricingAnalysis[$pricingField]=mb_substr(trim((string)$pricingInput[$pricingField]),0,40);} if(isset($pricingInput['estimatedRotationDays'])&&is_numeric($pricingInput['estimatedRotationDays']))$pricingAnalysis['estimatedRotationDays']=max(1,min(365,(int)$pricingInput['estimatedRotationDays'])); $contactName=trim((string)$contact['name']); $contactEmail=strtolower(trim((string)$contact['email'])); $contactPhone=trim((string)$contact['phone']); $location=trim((string)$v['location']); $fuel=trim((string)($v['fuel']??'')); $gearbox=trim((string)($v['gearbox']??'')); $dgtBadge=(string)($v['badge']??'');
        ...
        try {
            ...
            $metadata=['location'=>$location,'fuel'=>$fuel,'gearbox'=>$gearbox,'photo_slots'=>$imageMetadata,'publication_state'=>$publicationStatus,'evidence_level'=>'declarado','pricingAnalysis'=>$pricingAnalysis];
            $pdo->beginTransaction();
